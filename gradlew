...
#
# (2) This script is generated from the Gradle source. Changes to this script
...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        if
            case $arg in                                #(
              ...
              /?*)  t=${arg#/} t=/${t%%/*}              # look for remote root      #(
                    [ -e "$t" ] ;;                      # https://cygwin.com/ml/cygwin-announce/2006-01/msg00034.html
              *)    false ;;
            esac
        then
        ...
        # temporary variables.  (eg if 3 temporary variables are needed, then
        # the range is 1 to 3)
        set -- "$@" "$arg"
        shift
    done
